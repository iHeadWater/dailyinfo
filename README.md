...
│  │  config/scrapers.json → API 源（GitHub + HuggingFace）    │      │
│  └──────────────────────┬───────────────────────────────────┘      │
└─────────────────────────┼───────────────────────────────────────────┘
                          ▼
┌─────────────────────────────────────────────────────────────────────┐
...
API 类数据源（非 RSS）的配置文件，目前配置了 **4 个源**：

| 来源 | API | 说明 |
|------|-----|------|
| GitHub Trending | GitHub Search API | 近 2 天新项目，stars > 20 |
...
```json
{
  ...
  "category": "code",
  "enabled": true,
  "api_url": "https://api.github.com/search/repositories?q=created:>{two_days_ago}+stars:>20&sort=stars&order=desc&per_page=30",
  "auth_env": "GITHUB_TOKEN"
}
```

> **GitHub Token（可选）**：无 Token 时 API 限制 60 次/小时（足够日常使用）。如需更高限额，可在 `.env` 中添加 `GITHUB_TOKEN=ghp_xxx`。
...
- **GitHub Search API**：近 2 天新创建且 stars > 20 的仓库（全语言）
...
2. Loop Over Sources（循环每个 API 源）
   │
   ├─ 3. Fetch API（调用 GitHub/HuggingFace API）
   │     └─ 自动替换 URL 模板变量（{two_days_ago} 等）
   │
   ├─ 4. Has Items?（条件分支）
...
- **API 数据源**：GitHub Search API + HuggingFace API
- **容器编排**：Docker Compose

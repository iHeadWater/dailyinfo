...
class Item:
    ...
    extra: dict = field(default_factory=dict)  # source-specific fields (stars, likes, …)
...
class DataSource(ABC):
    def __init__(self, config: dict, defaults: dict):
        ...
        self.lookback_hours: int = config.get('lookback_hours') or defaults.get('lookback_hours', 24)
    ...
    def create(config: dict, defaults: dict, **ctx) -> 'DataSource':
        ...
        if t == 'rss':
            return RSSDataSource(config, defaults,
                                 db=ctx.get('db'),
...
class RSSDataSource(DataSource):
    def __init__(self, config, defaults, db=None, full_map=None, base_map=None):
        ...
        self.max_articles_per_batch: Optional[int] = config.get('max_articles_per_batch')
        self.max_batches: int = config.get('max_batches', 10)

    def fetch(self) -> list[Item]:
        if not self._db:
            return []
        fid = resolve_feed_id(self.config.get('url', ''), self._full_map, self._base_map)
        if not fid:
            return []
    ...
    def get_batches(self, items: list[Item]) -> list[list[Item]]:
        ...
        if not self.max_articles_per_batch:
            return [items]
        batches = [items[i:i + self.max_articles_per_batch]
...
class ScrapeDataSource(DataSource):
    ...
    def _fetch_github(self) -> list[Item]:
        ...
        articles = re.findall(r'<article\s+class="Box-row[^"]*">([\s\S]*?)</article>', resp.text)
        if not articles:
        ...
        for art in articles[:25]:
            ...
            repo_name = re.sub(r'\s+', '', re.sub(r'<[^>]+>', '', name_m.group(2))).strip() or repo_path
    ...
    def _parse_dlut_html(self, page_html: str) -> list[Item]:
        ...
        if date_format == 'dlut_news':
            ...
            for m in rgx.finditer(page_html):
                ...
                items.append(Item(title=title_raw.strip()[:100],
                                  date=dt.strftime('%Y-%m-%d') if dt else 'unknown', url=url))
            ...
            for m in rgx.finditer(page_html):
                ...
                if dt and dt < cutoff:
                    continue
                url = href if href.startswith('http') else base_url + href.lstrip('./')
                items.append(Item(title=title, date=dt.strftime('%Y-%m-%d') if dt else date_raw, url=url))
                if len(items) >= max_items:
                    break
            ...
            for m in rgx.finditer(page_html):
                ...
                if dt and dt < cutoff:
                    continue
                url = href if href.startswith('http') else base_url + href.lstrip('./')
                items.append(Item(title=title, date=dt.strftime('%Y-%m-%d') if dt else 'unknown', url=url))
                if len(items) >= max_items:
                    break
...
class APIDataSource(DataSource):
    def fetch(self) -> list[Item]:
        ...
        if method == 'POST':
            ...
            resp = requests.get(self.config['url'], params=params_str, headers=headers, timeout=30)
        resp.raise_for_status()
        data = resp.json()

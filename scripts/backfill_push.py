...
def main():
    ...
    parser.add_argument(
        "--targets",
        nargs="+",
        choices=list(BACKFILL_TARGETS.keys()),
    ...
    discord_channel_id = load_env("DISCORD_CHANNEL_PAPERS")
    if not discord_channel_id:
        log("ERROR: DISCORD_CHANNEL_PAPERS not set in .env (backfill targets papers)")
        sys.exit(1)
    ...
    for name in targets:
        ...
        date_range_start = datetime.datetime.fromtimestamp(rows[-1]["date"]).strftime(
            "%Y-%m-%d"
        )
        date_range_end = datetime.datetime.fromtimestamp(rows[0]["date"]).strftime(
            "%Y-%m-%d"
        )
        label = BACKFILL_TARGETS[name]["batch_label"]

        log(
            f"  {name}: {len(rows)} articles ({date_range_start} ~ {date_range_end}), generating briefing..."
        )

        article_list = "\n".join(f'{i+1}. {r["title"]}' for i, r in enumerate(rows))

        prompt = (
            one_line_tmpl.replace("{count}", str(len(rows)))
            .replace(
                "{display_name}",
                f"{display_name}（{label}：{date_range_start} ~ {date_range_end}）",
            )
        ...
        ok = discord_send(
            discord_token, discord_channel_id, full_content, dry_run=args.dry_run
        )
        if ok:

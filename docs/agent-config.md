...
- `0` — 所有 pipeline 健康，且本次至少处理了一份内容。
- `1` — 有 pipeline 崩溃或降级（DEGRADED），**或**本次没东西可干（幂等重跑、全部已存在）。

崩溃/降级与"没东西可干"目前共用 `1`，无法靠退出码区分，需要看日志里的 `Failed pipelines: [...]` 一行。因此调度器不宜把非零一律当告警；建议匹配该日志行，或改用 `--pipeline N` 单独跑关键 pipeline（此时退出码只反映该 pipeline 的健康状况，不受"无内容"影响）。
